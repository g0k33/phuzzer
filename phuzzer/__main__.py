...
def main():
    ...
    parser.add_argument('-g', '--grease-with', help="A directory of inputs to grease the fuzzer with when it gets "
                                                    "stuck.")
    parser.add_argument('-d', '--driller_workers', help="When the fuzzer gets stuck, drill with N workers.", type=int)
    parser.add_argument('-f', '--force_interval', help="Force greaser/fuzzer assistance at a regular interval (in "
                                                       "seconds).", type=float)
    ...
    parser.add_argument('-T', '--tarball', help="Tarball the resulting AFL workdir for further analysis to this file "
    ...
    parser.add_argument('--no-dictionary', help="Do not create a dictionary before fuzzing.", action='store_true',
                        default=False)
    ...
    parser.add_argument('--run-timeout', help="Number of milliseconds permitted for each run of binary", type=int,
                        default=None)
    parser.add_argument('--driller-timeout', help="Number of seconds to allow driller to run", type=int,
                        default=10 * 60)
    ...
    parser.add_argument('-p', '--phuzzer-type', '--fuzzer-type', help="Which phuzzer are you using: AFL, AFL_IJON, "
                                                                      "AFL++, Witcher, AFL_MULTICB, EMS.",
                        default=Phuzzer.AFL)
    ...
    fuzzer = Phuzzer.phactory(phuzzer_type=args.phuzzer_type,
                              ...
                              memory=args.memory, run_timeout=args.run_timeout, dictionary=built_dict,
